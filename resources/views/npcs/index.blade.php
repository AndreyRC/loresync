<x-app-layout>
    ...
    <x-card>
        <form method="GET" action="{{ route('npcs.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div class="flex-1">
                <x-input-label for="tag" :value="__('Filter by tag')" />
                <x-text-input id="tag" name="tag" type="text" class="mt-1 block w-full" :value="old('tag', $tag)" placeholder="{{ __('e.g. villain') }}" />
            </div>
            <div class="flex items-center gap-3">
